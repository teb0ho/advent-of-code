...
func laboratoriesPart2v2(input []string) {
				...
				for _, col := range nodes {
					if string(input[row][col]) == "|" && len(input)-1 == row {
						...
						for _, nk := range nodeKeys {
							...
							if slices.Contains(nodes, part) {
								if strings.Contains(nk, fmt.Sprintf(",%d", col)) {
									tree[nk] = []string{fmt.Sprintf("%d,%d", row, col)}
								}
							}
						}
						newNodes = append(newNodes, col)
	...
	count := countPaths(tree, fmt.Sprintf("1,%d", startPos))
